...
static ssize_t ts_readio(struct device *dev, struct device_attribute *attr, char *buff)
{
	unsigned int ioval;
	struct ntg_tsData *ts;

	ts  = private_ts;
	ioval  = (unsigned int)gpio_get_value(ts->spiDev->irq);

	printk(KERN_DEBUG "[ts_read_io]: gpio%d = %d", ts->spiDev->irq, ioval);
	return sprintf(buff, "gpio%d = %d", ts->spiDev->irq, ioval);

}
DEVICE_ATTR(ts_read, 0444, ts_readio, NULL);
...
static int ntrig_touch_sysfs_init(void)
{
	...
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_ts_read.attr);
	if (ret) {
		pr_err("[tp_ntg_err]sysfs_create_file failed\n");
		return ret;
	}
...
static void ntrig_touch_sysfs_remove(void)
{
	...
	sysfs_remove_file(android_touch_kobj, &dev_attr_ts_read.attr);
...
static bool ntg_CheckSum(char *buf, bool type)
{
	short i, loop_j;
	u16 sum = 0, oriSum;

	for (i = 0; i < ((type == true) ? 100: 16); i++)
		sum += buf[i];

	type == true ? 	memcpy((void *)&oriSum, (void *)&buf[104], 2) :
					memcpy((void *)&oriSum, (void *)&buf[20], 2);
	if (sum == oriSum) {
		return true;
	} else {
		pr_info("[ts]c_sum = %.2x, o_Sum = %.2x", sum, oriSum);
		for (loop_j = 0; loop_j < ((type == true) ? 8 : 2); loop_j++) {
	...
	if (buf[i] == 0xA5 && buf[i+1] == 0x5A &&
		...
		if (ntg->touchEvnt->header.channel == 0x30) {
...
#endif
				fCheckSum = ntg_CheckSum(&buf[nIndex], true);
				if (fCheckSum) {
			...
			case 0x02:
				...
				break;
			case 0x03:
				fCheckSum = ntg_CheckSum(&buf[nIndex], false);
				if (fCheckSum) {
...
static int ntrig_ts_probe(struct spi_device *dev)
{
	...
	for (i = 0; i < 12; i++) {
		intFlag = gpio_get_value(pdata->irq);
		if (intFlag) {
